//! Tests for Tower middleware composition with CloudClient
//!
//! These tests demonstrate how CloudClient integrates with Tower middleware
...
async fn test_tower_with_rate_limiting() {
    ...
    // Third request should be delayed (at least 400ms remaining from 500ms window)
    assert!(
